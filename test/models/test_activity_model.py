...
class TestActivity(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.activity = Activity(id=1, time_start=self.time_start, time_stop=self.time_stop,
                             window_class="TestClass", window_name="TestWindow",
  ...
  def test_activity_initialization(self):
    ...
    self.assertEqual(self.activity.time_start, self.time_start.isoformat())
    self.assertEqual(self.activity.time_stop, self.time_stop.isoformat())
  ...
  def test_activity_duration(self):
    ...
    expected_duration = 2 * 60 * 60  # 2 hours in seconds
  ...
  def test_activity_across_midnight(self):
    ...
    activity = Activity(time_start=time_start, time_stop=time_stop,
                        window_class="TestClass", window_name="TestWindow")
    self.assertEqual(activity.duration, 3600)  # 1 hour in seconds
